...
# Save where this .py file is located.(Assumes it is placed inside "scripts" folder in your Maya directory.)
SCRIPTPATH = os.path.dirname(os.path.abspath(__file__))
MAYAPATH = SCRIPTPATH.split("scripts")[0]
MODNAME = "CtrlColourPicker"
...
def onMayaDroppedPythonFile(*args):
    ...
    """
    shelf_name = "Rigging"  # Change to existing shelf name if you like.
    button_label = "Control Colour Picker"
    tooltip = "Launches the Control Colour Picker."
    icon_name = "CtrlColourPickerIcon.png"  # Your custom icon file name.
    ...
    if not mc.shelfLayout(shelf_name, exists=True):
        print(f"'{shelf_name}' Shelf not found. Please create it or change to an existing shelf (e.g., 'Polygons').")
        return
    ...
    for child in children:
        try:
            ...
            print(f"Could not find {child}. Skipping it.")

    # Command that will run your tool when the button is clicked.
    cmd = 'import CtrlColourPicker.CtrlColourPicker as cColPick; import imp; imp.reload(cColPick); cColPick.openWindow()'
